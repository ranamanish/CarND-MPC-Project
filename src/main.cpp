    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          *
          */
          // Transform the co-ordinates into Car Co-Ordinates
          size_t n_waylen = ptsx.size();
          auto car_ptsx = Eigen::VectorXd(n_waylen);
          auto car_ptsy = Eigen::VectorXd(n_waylen);
          for (unsigned int i = 0; i < n_waylen; i++ ) {
			  double dX = ptsx[i] - px;
			  double dY = ptsy[i] - py;
			  double minus_psi = 0.0 - psi;
			  car_ptsx( i ) = dX * cos( minus_psi ) - dY * sin( minus_psi );
			  car_ptsy( i ) = dX * sin( minus_psi ) + dY * cos( minus_psi );
          }
